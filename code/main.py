...
def main() -> None:
    ...
    if once:
        try:
            ...
            logger.error("La sesión de Colsubsidio ha expirado o es inválida: %s", e)
            current_time = time.time()
            if current_time - state.get("last_expiry_alert_time", 0.0) > 86400:
                msg = (
                    "⚠️ *[Alerta de Revisor de Natación]*\n\n"
                    "Tu sesión de Colsubsidio (cookie `sistema`) ha expirado o es inválida.\n"
                    "Por favor, abre la web de Colsubsidio en tu navegador o ejecuta el script local para renovarla."
                )
